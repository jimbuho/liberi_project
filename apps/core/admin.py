...
class UserAdmin(BaseUserAdmin):
    ...
    actions = ['force_delete_users']  # Acción personalizada de eliminación
    ...
    def force_delete_users(self, request, queryset):
        """
        Acción personalizada para eliminar usuarios forzadamente.
        Esto eliminará el usuario y todos sus objetos relacionados en cascada.
        """
        count = queryset.count()
        if count == 0:
            self.message_user(request, 'No se seleccionaron usuarios.', messages.WARNING)
            return
        
        # Eliminar usuarios (CASCADE se encargará de las relaciones)
        deleted_count = 0
        for user in queryset:
            try:
                ...
                self.message_user(
                    request,
                    f'Error eliminando {user.username}: {str(e)}',
                    messages.ERROR
                )
        ...
        if deleted_count > 0:
            self.message_user(
                request,
                f'✅ {deleted_count} usuario(s) eliminado(s) exitosamente.',
                messages.SUCCESS
            )
    
    force_delete_users.short_description = '🗑️ Eliminar usuarios seleccionados (forzar)'
